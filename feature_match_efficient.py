...
class Matcher():
    ...
    def get_features_label(self, feature_path):
        ...
        feature_label = path.join(path.split(path.split(feature_path)[0])[1], subject_id[:-4])
    ...
    def get_features(self, file):
        ...
        return np.asarray(all_features), np.asarray(all_subject_ids), np.asarray(all_labels)
    ...
    def get_indices_score(self, auth_or_imp):
        ...
        return np.transpose(np.vstack([x, y, np.round(
            self.matches[self.authentic_impostor == auth_or_imp], 6)]))
